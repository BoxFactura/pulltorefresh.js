...
"pending"},handler.refreshTimeout)},update:function update(handler){var iconEl=handler.ptrElement.querySelector("."+handler.classPrefix+"icon");var textEl=handler.ptrElement.querySelector("."+handler.classPrefix+"text");if(_shared.state==="refreshing")iconEl.innerHTML=handler.iconRefreshing;else iconEl.innerHTML=handler.iconArrow;if(_shared.state==="releasing")textEl.innerHTML=handler.instructionsReleaseToRefresh;if(_shared.state==="pulling"||_shared.state==="pending")textEl.innerHTML=handler.instructionsPullToRefresh;
if(_shared.state==="refreshing")textEl.innerHTML=handler.instructionsRefreshing}};function _setupEvents(){var _el;function _onTouchStart(e){var target=_shared.handlers.filter(function(h){return h.contains(e.target)})[0];_shared.enable=!!target;if(target&&_shared.state==="pending"){_el=_ptr.setupDOM(target);if(target.shouldPullToRefresh())_shared.pullStartY=e.touches[0].screenY;clearTimeout(_shared.timeout);_ptr.update(target)}}function _onTouchMove(e){if(!_shared.enable)return;if(!_shared.pullStartY){if(_el.shouldPullToRefresh())_shared.pullStartY=
